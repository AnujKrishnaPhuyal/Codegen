...
class Model(nn.Module):
    ...
    def get_xcode_vec(self, source_ids):
        ...
        # CodeGen uses causal attention, so we use a simple 1D attention mask
        out = self.encoder(source_ids, attention_mask=mask, output_hidden_states=True)

        # For GPT-2 based models, out[0] contains the hidden states or out.last_hidden_state
