...
def convert_examples_to_features(js,tokenizer,args):
    ...
    contrast_tokens = [tokenizer.bos_token] + contrast_tokens + [tokenizer.eos_token]
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.num_train_epochs): 
        ...
        avg_train_loss = round(np.mean(losses[-len(train_dataloader):]), 4)
        logger.info("***** Epoch {} Training Loss: {} *****".format(idx, avg_train_loss))
        
        results, eval_loss = evaluate(args, model, tokenizer, args.eval_data_file)
        
        logger.info("***** Epoch {} Validation Results *****".format(idx))
        logger.info("  Validation Loss = {}".format(round(eval_loss, 4)))
...
def main():
    ...
    # Set pad token for CodeGen (uses eos_token as pad_token)
